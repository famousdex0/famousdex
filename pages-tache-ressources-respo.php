...
if(isset($_POST['supp'])) {
    ...
    $query = "DELETE FROM tache_ressources_respo WHERE tache.id ='$id' ";
...
<li class="nav-item">
  <a class="nav-link " href="pages-tache-ressources-respo.php">
    ...
    <table class="table" style="width: max-content; box-shadow:1px 1px 10px black; position:absolute;  top:500px">
        ...
        <tbody>
            <?php
                ...
                $envoi = "SELECT * FROM tache_ressources_respo";
        ...
        <table class="table" style="width: max-content; margin: 0 auto;" >
              ...
              <tbody>
                ...
                $connexion = new mysqli("localhost", "root", "", "entreprise");
                /* PREPAREZ LA BASE DE DONNEES A RECHERCHER LES DONNEES */
                $envoi = "SELECT * FROM rapport_ressources_respo";
                $mess = $connexion->query($envoi);
                // Affichage des résultats
                if ($mess->num_rows > 0) {
                    while ($row = $mess->fetch_assoc()) {
                         ...
                         $result = $connexion->query($sql);
                          if ($result->num_rows > 0) {
                            ...
                            $nom =$_POST["Titre"];
                                                    ...
                                                    $date =$_POST["date"];
                                                    $req = "INSERT INTO tache (libelle, description, code, date) VALUES ('$nom', '$desc', '$category', '$date')"; 
                        ...
                        $connexion = new mysqli("localhost", "root", "", "entreprise");
                         $result = $connexion->query($sqll);
                          if ($result->num_rows > 0) {
                            ...
                            $datee =$_POST["datee"];
                                                    $descrip =$_POST["descrip"];
                                                    $requ = "INSERT INTO rapport (libelle, description, code, date) VALUES ('$name', '$descrip', '$categoryy', '$datee')"; 
