...
if(isset($_POST['supp'])) {
    ...
    $query = "DELETE FROM tache_marketing_respo WHERE tache_marketing_respo.id ='$id' ";
    $result = $connexion->query($query);

    // Vérifiez si la suppression a réussi
    if ($result) {
        // Redirigez l'utilisateur vers une page de confirmation ou une autre page appropriée
        header("Location: pages-tache-marketing-respo.php");
        exit();
    } else {
...
<head>
  ...
  <title>Pages / tache - Nicemarketing-respo Bootstrap Template</title>
...
<body>
  ...
  <aside id="sidebar" class="sidebar">
    ...
    <ul class="sidebar-nav" id="sidebar-nav">
      ...
      <li class="nav-item">
        <a class="nav-link collapsed" href="users-profile-marketing-respo.php">
          <i class="bi bi-person"></i>
          <span>Profil</span>
        </a>
...
<li class="nav-item">
  <a class="nav-link " href="pages-tache-marketing-respo.php">
    ...
    <table class="table" style="width: max-content; box-shadow:1px 1px 10px black; position:absolute;  top:500px">
        ...
        <tbody>
            <?php
                ...
                $envoi = "SELECT * FROM tache_marketing_respo";
        ...
        <table class="table" style="width: max-content; margin: 0 auto;" >
              ...
              <tbody>
                ...
                $connexion = new mysqli("localhost", "root", "", "entreprise");
                /* PREPAREZ LA BASE DE DONNEES A RECHERCHER LES DONNEES */
                $envoi = "SELECT * FROM rapport_marketing_respo";
                $mess = $connexion->query($envoi);
                // Affichage des résultats
                if ($mess->num_rows > 0) {
                    while ($row = $mess->fetch_assoc()) {
                         ...
                         $result = $connexion->query($sql);
                          if ($result->num_rows > 0) {
                            ...
                            $nom =$_POST["Titre"];
                                                    ...
                                                    $date =$_POST["date"];
                                                    $req = "INSERT INTO tache_marketing_respo (libelle, description, code, date) VALUES ('$nom', '$desc', '$category', '$date')"; 
                        ...
                        $connexion = new mysqli("localhost", "root", "", "entreprise");
                         $result = $connexion->query($sqll);
                          if ($result->num_rows > 0) {
                            ...
                            $datee =$_POST["datee"];
                                                    $descrip =$_POST["descrip"];
                                                    $requ = "INSERT INTO rapport_marketing_respo (libelle, description, code, date) VALUES ('$name', '$descrip', '$categoryy', '$datee')"; 
